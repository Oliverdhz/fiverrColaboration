...
const CarIcon = styled(Image)`${tw`w-5/6 h-full object-contain pointer-events-none`}`;
...
const SubTitle = tw.h3`text-xl font-bold`;
const SelectedButtonText = tw.p`text-xl font-bold ml-2 flex-wrap break-words`;
...
const StyleSelector = ({ title, bodyTypes, isCarCategoryPage }) => {
  const [selectedStyle, setSelectedStyle] = useState('');
  const [selectedButtonText, setSelectedButtonText] = useState('');
  const [breadcrumbItems, setBreadcrumbItems] = useState(['Home', 'Ford', 'Corolita', '2012', 'Styles']);
  const [styles, setStyles] = useState(bodyTypes?.map((bodyType) => ({ ...bodyType, isSelected: false })));
  ...
  const handleNextClick = () => {
    ...
    updatedBreadcrumbItems[4] = selectedStyle;
  ...
  return (
    <React.Fragment>
      ...
      <TitleContainer>
        ...
        <SubTitleContainer>
          <SubTitle>2008 Honda S2000</SubTitle>
          <SelectedButtonText>{selectedButtonText}</SelectedButtonText>
        </SubTitleContainer>
      </TitleContainer>
      <MainContainer>
        <Container>
          ...
          <ButtonGroup>
            {styles?.map((style) => (
              ...
              >
                ...
                {style.img && (
                  <ImageContainer onClick={() => handleStyleClick(style.name)}>
                    <CarIcon src={style.img} alt={`Imagen ${style.name}`} />
                  </ImageContainer>
                )}
              </Button>
            ))}
